#! /usr/bin/ruby
#
#Use like: ruby scrape_kayak.rb f PIT SFO 03/21/2010
#ruby scrape_kayak.rb (flight) (from) (to) (date with full year)
require 'net/http'
require 'rexml/document' 
require 'uri'

# @@token ='Y9A524rluRlgcUhElQAAWw'
#@@token ='PeD3Ee3lrvtWRlXN1ovPNw'
# @@token ='6KFp5rAwII0wq1rOBMetjg'
# @@token ='OJYtFCkIUosvzq6ytt6MSg'

# @@token ='c6NEG79BvXAcIaffeyoprQ'
@@token ='PgMATgLPmweu68f51HrRLQ'
...
def start_flight_search(sid,
                        ...
                        ret_date,
                        travelers)
  url = "/s/apisearch?basicmode=true&oneway=y&origin=#{origin}&destination=#{destination}&destcode=&depart_date=#{dep_date}&depart_time=a&return_date=#{ret_date}&return_time=a&travelers=#{travelers}&action=doflights&apimode=1&_sid_=#{sid}"
